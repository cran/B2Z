...
function(x)
  ...
  cla <- class(x)
  if(class(x)=="mh" || class(x)=="gibbs")
     {index <- seq(x$burnin, x$NUpd, by = x$lag)}
  else
     {index <- seq(1:length(x$Beta))}


   if(x$indep){parms <- cbind(x$tauN[index], 0, x$tauF[index], x$logCN[index,], x$logCF[index,])}
   else{parms <- cbind(x$tauN[index], x$tauNF[index], x$tauF[index], x$logCN[index,], x$logCF[index,])}
   n <- length(x$times)- 1
  ...
  densup <- 1.20*max(z$density, density(Beta)$y, dnorm(mean(Beta),mean(Beta),sd(Beta)))
  ...
  densup <- 1.20*max(z$density, density(Q)$y, dnorm(mean(Q),mean(Q),sd(Q)))
  hist(Q, xlab = "Q", xlim=c(lowQ,upQ),ylim=c(0,densup), freq=FALSE, main = "Empirical posterior distribution of Q")
  ...
  densup <- 1.20*max(z$density, density(G)$y, dnorm(mean(G),mean(G),sd(G)))
  hist(G, xlab = "G", xlim=c(lowG,upG), ylim=c(0,densup),freq=FALSE, main = "Empirical posterior distribution of G")
  ...
  densup <- 1.20*max(z$density, density(TauN)$y, dnorm(mean(TauN),mean(TauN),sd(TauN)))
  ...
  if(class(x)=="metrop" || class(x)=="gibbs")
   {  
   postscript("Trace_and_ACF1.eps")
   par(mfrow=c(2,3))
   ...
   if(x$indep)
      ...
      acf(x$tauN, main=expression(paste("ACF for ",tau[N])))
      acf(x$tauF, main=expression(paste("ACF for ",tau[F])))
      }
   else
      {
      par(mfrow=c(2,3))
      ts.plot(x$tauN, ylab=expression(tau[N]), xlab="Update")
      ts.plot(x$tauF, ylab=expression(tau[F]), xlab="Update")
      ts.plot(x$tauNF, ylab=expression(tau[NF]), xlab="Update")
      acf(x$tauN, main=expression(paste("ACF for ",tau[N])))
      acf(x$tauF, main=expression(paste("ACF for ",tau[F])))
      acf(x$tauNF, main=expression(paste("ACF for ",tau[NF])))
      }
   dev.off()
   }
   options(warn=0)
